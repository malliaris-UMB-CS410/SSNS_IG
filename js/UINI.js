...
class UINI {
    ...
    constructor(ui, id_str, indicate_params_changed) {
        ...
        if (!this.yields_valid_value)
            throw new Error("Derived UINI must define yields_valid_value()");
        if (!this.parse_from_str)
            throw new Error("Derived UINI must define parse_from_str()");
