...
class ModelCalc {
    ...
    constructor() {
        if (!this.model_is_stoch)
            throw new Error("Derived ModelCalc must define model_is_stoch()");
    }
}
...
class Params {
    ...
    constructor() {
        if (!this.push_vals_to_UI)
            throw new Error("Derived Params must define push_vals_to_UI()");
        if (!this.get_info_str)
            throw new Error("Derived Params must define get_info_str()");
    }
}
...
class Coords {
    ...
    constructor() { // see discussion of # args at definition of abstract Coords()
        ...
        if ((arguments.length == 2) || (arguments.length == 4)) {
            ...
            throw new Error("ERROR 490122: Invalid number of arguments to Coords(): " + arguments.length + ".  (Should be 2 or 4.)  Exiting...");
    }
}
...
class Trajectory {
    ...
    constructor(sim) {
        ...
        if (!this.gmc)
            throw new Error("Derived Trajectory must define gmc()");
        if (!this.gp)
            throw new Error("Derived Trajectory must define gp()");
        if (!this.gc_ic)
            throw new Error("Derived Trajectory must define gc_ic()");
        if (!this.gc_nv)
            throw new Error("Derived Trajectory must define gc_nv()");
        if (!this.get_max_num_t_steps)
            throw new Error("Derived Trajectory must define get_max_num_t_steps()"); // ST-specific to sensibly limit memory usage
    ...
    check_integrity() {
        ...
        if (!csi_t_consis)
            throw new Error("ERROR 823826: mismatch between values of t and csi!  Exiting...");
        if (!t_last_edge_consis)
            throw new Error("ERROR 823827: mismatch between values of t_last of last seg and t_edge!  Exiting...");
    ...
    output_curr_vals() {
        ...
        if (this.get_cx().output)
            this.get_cx().output(); // if applicable Coords class has .output() method, use it!
    }
    ...
    get_si(t_val) { // si = segment index
        let i = 0;
        while (t_val > this.segs[i].t_last)
            i++;
        return i;
    }
