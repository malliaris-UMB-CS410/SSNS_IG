...
class PlotTypeXT extends PlotType {
    ...
    static flot_t_symb_opts = {
        ...
        color: "#000000"
    }; // will be plotted "behind" all other data
    static flot_partial_seg_opts = {
        lines: {
            show: true
        },
        points: {
        ...
        color: "purple"
    }; // purple just a placeholder
    static flot_seg_connector_opts = {
        lines: {
            show: true
        },
        color: "purple"
    }; // purple just a placeholder... will be overwritten
    static flot_initial_gen_opts_XT = {
        xaxis: {
            tickDecimals: 0
        }
    };
    ...
    constructor() {
        ...
        if (!this.get_flot_data_series)
            throw new Error("Derived PlotTypeXT must define get_flot_data_series()");
        if (!this.get_flot_gen_opts)
            throw new Error("Derived PlotTypeXT must define get_flot_gen_opts()");
...
class PlotTypeXT_custom extends PlotTypeXT {
    ...
    get_flot_data_series(t) {
        ...
        let fxn_obj = t => {
            return this.trj.get_x(t).x_indiv.get(0);
        };
...
class PlotTypeXT_SP extends PlotTypeXT_rect {
    ...
    get_flot_data_series(t) {
        ...
        for (let EMi = 0; EMi < Coords_SP.num_IEM.v; EMi++) {
            let fxn_obj = t => {
                return this.trj.get_x(t).x_indiv.get(EMi);
            };
            this.assemble_data_by_seg(fds, fxn_obj, this.t_i, this.t_f);
        }
        return fds;
