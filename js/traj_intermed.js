...
class ModelCalc_Stoch extends ModelCalc {
    ...
    constructor() {
        ...
        this.discunif_rng = discreteUniform.factory({
            'seed': ModelCalc_Stoch.rng_seed.v
        });
        this.binom_rng = binomial.factory({
            'seed': ModelCalc_Stoch.rng_seed.v
        });
    ...
    test_rngs() {
        ...
        let TESTunif01_rng = randu.factory({
            'seed': ModelCalc_Stoch.rng_seed.v
        });
        let num_below = 0;
...
class Trajectory_Stoch extends Trajectory {
    ...
    constructor(sim) {
        ...
        super(sim);
        if (!this.set_rng_states_from_edge_vals)
            throw new Error("Derived Trajectory_Stoch must define set_rng_states_from_edge_vals()");
    }
}
...
class ModelCalc_SP extends ModelCalc_Stoch {
    ...
    constructor() {
        ...
        if (!this.get_P_step_R)
            throw new Error("Derived ModelCalc_SP must define get_P_step_R()");
        if (!this.get_P_step_L)
            throw new Error("Derived ModelCalc_SP must define get_P_step_L()");
    }
    ...
    get_x_new(p, x) {
        ...
        if (rn01 < P_step_R)
            return (x + 1);
        else if (rn01 < (P_step_R + P_step_L))
            return (x - 1);
        else
            return x;
    }
...
class Coords_SP_finite extends Coords_SP {
    ...
    constructor(...args) { // see discussion of # args at definition of abstract Coords()
        ...
        if (Coords_SP.num_IEM.v > 0) {
            this.x_indiv = zeros([Coords_SP.num_IEM.v], {
                'dtype': 'int32'
            });
        }
        if (Coords_SP.num_GEM.v > 0) {
            this.H_x_group = zeros([num_x], {
                'dtype': 'int32'
            });
        }
